...
const myTimeout = setTimeout(function () {
  setInterval(async function () {
    await $("#list-view-2 article:not(.filtered,.filtering), .list-view__content article:not(.filtered,.filtering)").each(async function () {
      ...
      let name = document.querySelector("#" + art_id + " .ui-post-creator__author") !== null ? document.querySelector("#" + art_id + " .ui-post-creator__author").text : await getNameFromMenu(art_id);
      if (document.querySelector("#" + art_id + " .ui-post-creator") === null) {
        $("#" + art_id + " .post-header__left").append(`<span>| <a class="user-link" href="https://9gag.com/u/${name}">@${name}</a></span>`);
        $("#" + art_id + " .post-meta.mobile").append(`<br/><span> <a class="user-link" href="https://9gag.com/u/${name}">@${name}</a></span>`);
      }
      ...
      if (document.querySelector("#" + art_id + " .post-meta__list-view .name") !== null) post_tags.push(document.querySelector("#" + art_id + " .post-meta__list-view .name").text.toLowerCase());
      ...
      if (settings.show_days || settings.min_days > 0) {
        ...
        if (document.querySelector("#" + art_id + " .ui-post-creator") !== null) $("#" + art_id + " .ui-post-creator").append("| " + diff + " days");
        else {
          $("#" + art_id + " .post-header__left").append("<span>| " + diff + " days</span>");
          $("#" + art_id + " .post-meta.mobile").append("<span>| " + diff + " days</span>");
        }
        ...
        if (settings.spammers && json.data.posts.length >= 10) {
          ...
          if (diffAve < diffset) {
            if (settings.hide_spammers) {
              ...
              if (document.querySelector("#" + art_id + " .ui-post-creator") !== null) $("#" + art_id + " .ui-post-creator").append(`<span class="spammer-label">| SPAMMER</span>`);
              else {
                $("#" + art_id + " .post-header__left").append(`<span class="spammer-label">| SPAMMER</span>`);
                $("#" + art_id + " .post-meta.mobile").append(`<span class="spammer-label">| SPAMMER</span>`);
              }
            }
          }
        }
        ...
        if (downvotes !== null) {
          ...
          $("#" + art_id + " .post-vote").append(`<span class="post-vote__text downvote upvote">${downvotes}</span>`);
